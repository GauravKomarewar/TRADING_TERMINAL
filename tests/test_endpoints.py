...
def main():
    if not PASSWORD:
        ...
        print("Set it with: export PASSWORD=<your-dashboard-password>")
        sys.exit(1)
    ...
    print("   Cookies: " + str({k: "***" for k in session.cookies.get_dict()}))
    ...
    # Fetch a valid current expiry dynamically
    _expiry_result = test_endpoint("GET", "/dashboard/option-chain/active-expiries?exchange=NFO&symbol=NIFTY", "Fetch NIFTY Expiries for dynamic tests")
    _nifty_expiry = None
    if _expiry_result.get("data") and isinstance(_expiry_result["data"], list) and len(_expiry_result["data"]) > 0:
        _nifty_expiry = _expiry_result["data"][0]
    elif _expiry_result.get("data") and isinstance(_expiry_result["data"], dict):
        _nifty_expiry = (_expiry_result["data"].get("expiries") or [None])[0]

    if _nifty_expiry:
        test_endpoint("GET", f"/dashboard/option-chain?exchange=NFO&symbol=NIFTY&expiry={_nifty_expiry}", f"Option Chain for NIFTY ({_nifty_expiry})")
        test_endpoint("GET", f"/dashboard/option-chain/nearest?exchange=NFO&symbol=NIFTY&expiry={_nifty_expiry}&target=26000&metric=ltp", f"Nearest Option for NIFTY ({_nifty_expiry})")
    else:
        print("   [SKIP] Could not determine a valid NIFTY expiry; skipping option-chain detail tests")
