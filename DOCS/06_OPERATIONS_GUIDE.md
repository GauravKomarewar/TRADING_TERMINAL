...
python3 -c "
from shoonya_platform.tools.cleanup import cleanup
cleanup()
"
```
...
The `trading_weekend_check.timer` runs `scripts/weekend_market_check.py` on weekends to:
...
|--------|----------|---------|
| `verify_orders.py` | `scripts/` | Cross-reference local DB with broker orders |
...
| `cleanup.py` | `shoonya_platform/tools/` | Database and log cleanup |
| `test_webhook.py` | `shoonya_platform/tools/` | Test webhook with sample payload |
| `test_place_order.py` | `shoonya_platform/tools/` | Test order placement |
