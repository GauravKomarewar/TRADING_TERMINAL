...
LOCATION: utilities/cleanup.py
SETUP:    Run bootstrap.py — it configures this utility automatically.

MANUAL SETUP (Linux/EC2):
    chmod +x ~/shoonya_platform/utilities/cleanup.py
    sudo ln -sf ~/shoonya_platform/utilities/cleanup.py /usr/local/bin/cleanup
...
# utilities/cleanup.py → parents[1] = project root
...
def stop_services(services: list[str]):
    ...
    for service in services:
        ...
        if result.returncode != 0:
            print(f"  ❌ Failed to stop {service}: {result.stderr.strip()}")
            all_ok = False
    if all_ok:
        print("✅ Services stopped")
    else:
        print("⚠️  Some services failed to stop — check output above")
